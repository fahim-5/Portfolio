...
const portfolioService = {
  ...
  getSectionData(section) {
    try {
      ...
      const key =
        STORAGE_KEYS[section.toUpperCase()] ||
        `portfolio_${section.toLowerCase()}`;
  ...
  saveSectionData(section, data) {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("localDataChanged", {
          detail: { key },
        })
      );
  ...
  incrementPortfolioViews() {
    try {
      const views = parseInt(
        localStorage.getItem(STORAGE_KEYS.VIEWS) || "0",
        10
      );
  ...
  async fetchHeroData() {
    try {
      ...
      try {
        ...
        console.error(
          "Error fetching from public endpoint, trying admin endpoint:",
          publicError
        );
  ...
  async updateHeroData(heroData) {
    try {
      ...
      console.log(
        "portfolioService: Transformed backend data for update:",
        backendData
      );
  ...
  async requestPasswordReset(email) {
    try {
      ...
      if (response.data && response.data.success) {
        // Store the email temporarily for next steps
  ...
  async verifyResetCode(code) {
    try {
      ...
      if (!email) {
        return { success: false, error: "Reset email not found" };
      }
  ...
  async completePasswordReset(newPassword) {
    try {
      ...
      if (!localStorage.getItem("resetVerified")) {
    ...
    try {
      ...
      try {
        ...
        if (!serverCheckResponse.ok) {
          console.warn(
            "Server check failed with status:",
        ...
        throw new Error(
          "Backend server may not be running. Please start the server."
        );
      }
      ...
      try {
        ...
        if (error.response) {
          ...
          if (
            ...
            console.error(
              "Received HTML instead of JSON. Server may have crashed."
            );
            throw new Error(
              "Server returned HTML instead of JSON. Backend may have an error."
            );
          }
          ...
          console.error("No response received:", error.request);
          throw new Error(
            "No response from server. Backend may not be running."
          );
        }
    ...
    try {
      ...
      try {
        ...
        if (publicRes.ok) {
          ...
          console.log(
            "Successfully fetched skills from public endpoint:",
            skillsData
          );
        } else {
          console.log("Public endpoint returned status:", publicRes.status);
          throw new Error(
            `Public endpoint returned status: ${publicRes.status}`
          );
        }
      } catch (publicError) {
        ...
        try {
          ...
          if (adminRes.ok) {
            ...
            console.log(
              "Successfully fetched skills from admin endpoint:",
              skillsData
            );
          } else {
            console.log("Admin endpoint returned status:", adminRes.status);
            throw new Error(
              `Admin endpoint returned status: ${adminRes.status}`
            );
          }
        } catch (adminError) {
  ...
  async createSkill(skillData) {
    try {
      ...
      console.log(
        "portfolioService: Create skill failed - success flag not true"
      );
      return false;
    } catch (error) {
      ...
      if (error.response) {
        console.error(
          "portfolioService: Server responded with error:",
          error.response.data
        );
      }
      return false;
  ...
  async updateSkill(id, skillData) {
    try {
      console.log(
        `portfolioService: Updating skill ${id} with data:`,
        skillData
      );
      ...
      console.log(
        "portfolioService: Update skill failed - success flag not true"
      );
      return false;
    } catch (error) {
  ...
  async deleteSkill(id) {
    try {
      ...
      console.log(
        "portfolioService: Delete skill failed - success flag not true"
      );
      return false;
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (publicRes.ok) {
          ...
          console.log(
            "Successfully fetched projects from public endpoint:",
            projectsData
          );
        } else {
          console.log("Public endpoint returned status:", publicRes.status);
        ...
        try {
          ...
          if (adminRes.ok) {
            ...
            console.log(
              "Successfully fetched projects from admin endpoint:",
              projectsData
            );
          } else {
            console.log("Admin endpoint returned status:", adminRes.status);
      ...
      if (projectsData && Array.isArray(projectsData)) {
        ...
        const normalizedData = projectsData.map((project) => ({
          ...
          image:
            project.image ||
            project.imageUrl ||
            "https://placehold.co/800x600/gray/white?text=No+Image",
  ...
  async fetchPicturesData() {
    try {
      ...
      console.log(
        `Response status from ${baseURL}/api/pictures:`,
        response.status
      );

      if (!response.ok) {
        throw new Error(
          `Failed to fetch pictures: ${response.status} ${response.statusText}`
        );
      }
  ...
  async createPicture(pictureData) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to create picture: ${response.status} ${response.statusText}`
        );
      }
  ...
  async updatePicture(id, pictureData) {
    try {
      ...
      console.log(
        `Response status from ${baseURL}/api/pictures/${numericId}:`,
        response.status
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to update picture: ${response.status} ${response.statusText}`
        );
      }
  ...
  async deletePicture(id) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Error response:", errorData);
        throw new Error(
          `Failed to delete picture: ${response.status} ${response.statusText}`
        );
      }
  ...
  async fetchReferencesData() {
    try {
      ...
      console.log(
        `portfolioService: Fetching references from ${baseURL}${endpoint}`
      );
      ...
      try {
        ...
        console.log(
          `portfolioService: Response status from ${endpoint}:`,
          response.status
        );

        if (!response.ok) {
          throw new Error(
            `Failed to fetch references: ${response.status} ${response.statusText}`
          );
        }

        const data = await response.json();
        console.log(
          "portfolioService: References data received from server:",
          data
        );
  ...
  async createReference(referenceData) {
    try {
      ...
      console.log(
        `Response status from ${baseURL}/api/references:`,
        response.status
      );
  ...
  async updateReference(id, referenceData) {
    try {
      ...
      console.log(
        `Response status from ${baseURL}/api/references/${numericId}:`,
        response.status
      );
      ...
      if (!response.ok) {
        ...
        console.error("Error response:", errorData);
        throw new Error(
          `Failed to update reference: ${response.status} ${response.statusText}`
        );
      }
  ...
  async deleteReference(id) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Error response:", errorData);
        throw new Error(
          `Failed to delete reference: ${response.status} ${response.statusText}`
        );
      }
